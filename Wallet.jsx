...
import { utf8ToBytes } from "ethereum-cryptography/utils";
 ...
 async function Wallet({ address, setAddress, balance, setBalance,privateKey,setPrivateKey, message,setMessage }) {
  ...
  function change(evt){
    ...
    const hashMessage = toHex(keccak256(bytes));   
  }
    ...
    const address = publicKey.slice(0,10);
  ...
  const signature =toHex(secp.secp256k1.sign(hashMessage ,privateKey,{ recovered: true })) 
  const data = {signature,hashMessage,publicKey}
  const options = {
    method: 'POST',
    headers: {
